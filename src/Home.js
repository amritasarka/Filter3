...
const Home = () => {
    ...
    const apply = () => {
        ...
        for (let i = 0; i < posts.length; i++) {
            const string = posts[i]
            const comment = string.body
            const likes = string.postId
            const followers = string.user.id
            const name = string.user.username
    
            const obj = { comment: comment, likes: likes, followers: followers, name: name }
   ...
   useEffect(() => {
    ...
    const sliderArray = [{ id: 1, value: likevalue, handlechange: likehandleChange, min: minLikes, max: maxLikes, name: "likes", },
    { id: 2, value: followervalue, handlechange: followerhandleChange, min: minFollowers, max: maxFollowers, name: "Followers", }];
    ...
    const tableheader = [{ id: 1, name: "Name" },
    { id: 2, name: "Likes", other: "likes", }, { id: 3, name: "Follower", other: "followers", }, { id: 3, name: "Comment", }]
            ...
            <div className='maindiv'>
                    ...
                    <thead>
                        <tr>
                            {tableheader.map(item => (
                                <th key={item} onClick={() => handleSort(item.other)}>{item.name}</th>
                            ))}
                        </tr>
                ...
                {sliderArray.map((item) => (
                    ...
                    <><p className='pslider'>{item.name}</p>
                        <Slidercomponent value={item.value} onChange={item.handlechange} min={item.min} max={item.max} />
                    </>))}
